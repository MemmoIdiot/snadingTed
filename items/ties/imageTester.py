...
async def uploadFoto(driver, absPath):
    driver.find_element(By.XPATH,
                        "//div[@id='photos']/input").send_keys(absPath)
...
def job():
    ...
    for tie in listdir(Data.PATH):
        ...
        for i, pic in enumerate(sorted(listdir(join(Data.PATH, tie))), 1):
            LOOP.run_until_complete(
                uploadFoto(driver, join(Data.PATH, tie, pic)))
            # * (Carica 🇮🇹)
        '''
        sleep(5)
        ...
        try:
            ...
            print("Brand is already selected")
        ...
        driver.find_element(By.CSS_SELECTOR,
                            config.PRICE).send_keys(Data.PRICE)
